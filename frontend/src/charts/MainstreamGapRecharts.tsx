...
import { CHART_COLORS, PERSONAL_UNIVERSE_SIZE, maxGlobalRank, rankFraction } from "./shared";
...
export default function MainstreamGapRecharts({ rows }: { rows: PersonalVsGlobalRow[] }) {
  ...
  const data = useMemo<GapDatum[]>(() => {
    ...
    const withGap = filtered.map((r) => ({
      ...
      gap: rankFraction(r.global_rank, globalMax) - rankFraction(r.personal_rank, PERSONAL_UNIVERSE_SIZE),
  ...
  const rowHeight = 28;
  const height = Math.max(1, data.length) * rowHeight + 40;
    ...
    <div>
      ...
      <div className="chart-legend">
        <span className="chart-legend-item">
          <span className="chart-legend-dot" style={{ background: CHART_COLORS.divergingNegative }} />
          Bigger favorite for you than its fame
        </span>
        <span className="chart-legend-item">
          <span className="chart-legend-dot" style={{ background: CHART_COLORS.divergingPositive }} />
      ...
      <div className="chart-canvas" style={{ height }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ top: 8, right: 24, bottom: 8, left: 8 }}>
            ...
            <Tooltip content={GapTooltip} cursor={{ fill: "var(--spotify-elevated-hover)" }} />
            <Bar
              dataKey="gap"
              radius={4}
              ...
              shape={(props: BarShapeProps) => {
                ...
                const fill = datum.gap >= 0 ? CHART_COLORS.divergingPositive : CHART_COLORS.divergingNegative;
      ...
      {data.length === 0 && <p className="status-text">No overlap data for this time range yet.</p>}
    </div>
  );
}
